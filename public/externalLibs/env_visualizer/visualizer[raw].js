...
(function (exports) {
    ...
    if (Array.isArray(dataObject[1])) {
      ...
      if (cycleDetected) {
        ...
        if (yCoord - 22 == newStartY) {
          ...
          drawLine(
            ...
            newStartY,
            newStartX,
            newStartY - DATA_UNIT_HEIGHT / 2 - 10
          );
          drawLine(
            arrowContext,
            newStartX,
          ...
          drawArrow(
            ...
            newStartY - DATA_UNIT_HEIGHT / 2 - 10,
            xCoord,
            yCoord - DATA_UNIT_HEIGHT - 2
          ); // -2 accounts for buffer
          ...
          // arrow points down
          const boxEdgeCoord = inBoxCoordinates(
            newStartX,
            newStartY,
            xCoord,
            yCoord - DATA_UNIT_HEIGHT,
            false
          );
      ...
      if (startX + (3 / 4) * DATA_UNIT_WIDTH <= wrapperData[1].x + 4 * FNOBJECT_RADIUS) {
        drawLine(
          ...
          startY + DATA_UNIT_HEIGHT / 2,
          startX + (3 / 4) * DATA_UNIT_WIDTH,
          wrapperData[1].y + 2 * FNOBJECT_RADIUS
        );
        drawLine(
          arrowContext,
          startX + (3 / 4) * DATA_UNIT_WIDTH,
        ...
        drawArrow(
          ...
          wrapperData[1].x + 4 * FNOBJECT_RADIUS,
          wrapperData[1].y,
          wrapperData[1].x + 2 * FNOBJECT_RADIUS,
          wrapperData[1].y
        );
    ...
    if (Array.isArray(dataObject[0])) {
      ...
      if (startX + DATA_UNIT_WIDTH / 4 <= wrapperData[0].x + 4 * FNOBJECT_RADIUS) {
        drawLine(
          ...
          startY + DATA_UNIT_HEIGHT / 2,
          startX + DATA_UNIT_WIDTH / 4,
          wrapperData[0].y + 2 * FNOBJECT_RADIUS
        );
        drawLine(
          arrowContext,
          startX + DATA_UNIT_WIDTH / 4,
        ...
        drawArrow(
          ...
          wrapperData[0].x + 4 * FNOBJECT_RADIUS,
          wrapperData[0].y,
          wrapperData[0].x + 2 * FNOBJECT_RADIUS,
          wrapperData[0].y
        );
        // duplicate arrow in hoveredLayer
        var hoverContext = hoveredLayer.scene.context;
  ...
  function positionItems(frames) {
    ...
    fnObjects.forEach(function (fnObject) {
      ...
      if (Array.isArray(parent)) {
        ...
        fnObject.x = parent.x + parent.width + OBJECT_FRAME_RIGHT_SPACING;
        fnObject.y =
          parent.y +
  ...
  function draw_env(context) {
    ...
    function parseInput(allFrames, envs) {
      ...
      for (let i = environments.length - 1; i >= 0; i--) {
        ...
        } else if (environment.name == 'programEnvironment' && environment.tail.name == 'global') {
      ...
      frames.forEach(function (frame) {
        ...
        for (e in elements) {
          if (typeof elements[e] == 'function' && !fnObjects.includes(elements[e])) {
            initialiseFrameFnObject(elements[e], frame);
            fnObjects.push(elements[e]);
          } else if (typeof elements[e] == 'object' && !dataObjects.includes(elements[e])) {
            dataObjects.push(elements[e]);
            dataObjectWrappers.push(initialiseDataObjectWrapper(e, elements[e], frame));
    ...
    for (e in elements) {
      if (true) {
        ...
        if (literals.includes(typeof elements[e])) {
          currLength =
            e.length + elements[e].toString().length + (typeof elements[e] == 'string' ? 2 : 0);
  ...
  function findElementNamePosition(elementName, frame) {
    ...
    for (let i = 0; i < lineNo; i++) {
      if (isFunction(Object.values(frame.elements)[i])) {
        ...
        const parent =
          dataObjectWrappers[dataObjects.indexOf(Object.values(frame.elements)[i])].parent;
        if (parent == frame) {
  ...
  function isEmptyFrame(frame) {
    ...
    return !hasObject && frame.children.length == 0 && Object.keys(frame.elements).length == 0;
  }
